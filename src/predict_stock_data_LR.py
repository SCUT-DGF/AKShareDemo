...
"""
    函数功能介绍:
    1. get_single_daily_stock_data(symbol, start_date, end_date) 获取单个股票的日行情数据
    2. load_stock_symbols(file_path) 读取JSON文件并提取股票代码
    3. get_multiple_daily_stock_data(stock_symbols, start_date, end_date) 获取多个股票的日行情数据，并将他们合并成一个DataFrame
    4. preprocess_data(all_stock_data, pre_days) 数据预处理
    5. select_best_features(X, y, method, k) 特征选择
    6. train(X_best_features, X_lately, y, test_size=0.1, random_state=42) 训练模型
    7. test(model, X_test, y_test) 测试模型
    8. predict(model, X_lately) 预测模型
    9. plot_predict_graph(model, X_lately, y_lately, title) 绘制预测图
"""
...
def train(X_best_features, y, test_size=0.1, random_state=42):
    """
    训练模型
...
def predict(model, X_lately, scaler):
    ...
    if 'Symbol' in X_lately.columns:
        ...
        # 标准化训练集特征（包括股票代码列）
        X_lately = scaler.fit_transform(X_lately)
...
def plot_predict_graph(stock_data, forecast_set):
    ...
    plt.gca().xaxis.set_tick_params(labelsize=5)
    #
    # 保存图像到本地的paint目录下，加上股票代码作为文件名
    plt.savefig('../paint/' + stock_data['Symbol'].values[0] + '.png')
...
if __name__ == "__main__":
    ...
    # JSON文件路径
    json_file_path = '../data/stock_data/sh_a_stocks.json'
    ...
    # # 保存预测结果需要的数据stock_data、预测结果forecast_set
    # # 将 stock_data 保存为 CSV 文件
    # stock_data_df = pd.DataFrame(stock_data)
    #
    # forecast_df = pd.DataFrame(forecast_set)
    #
    # stock_data_df.to_csv('./csv/stock_data.csv', index=False)
    #
    # # 将 forecast 保存为 CSV 文件
    # forecast_df.to_csv('./csv/forecast.csv', index=False)
