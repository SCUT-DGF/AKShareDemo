...
def get_weekly_report(func, basic_name, stock_dict, flag, args, base_path='./stock_data/company_data',
    ...
    data_file = os.path.join(os.path.join(report_path, f"all_weekly_report_{report_date}.json"))
    up_3_days_file = os.path.join(report_path, f"{basic_name}_up_3_days_{report_date}.json")
    up_4_days_file = os.path.join(report_path, f"{basic_name}_up_4_days_{report_date}.json")
    up_5_days_file = os.path.join(report_path, f"{basic_name}_up_5_days_{report_date}.json")
...
def check_daily_up(func, basic_name, stock_dict, flag, args, base_path='./stock_data/company_data',
    ...
    up_range13_file = os.path.join(report_path, f"{basic_name}_up_range13_{report_date}.json")
    up_range35_file = os.path.join(report_path, f"{basic_name}_up_range35_{report_date}.json")
    up_range57_file = os.path.join(report_path, f"{basic_name}_up_range57_{report_date}.json")
    up_range710_file = os.path.join(report_path, f"{basic_name}_up_range710_{report_date}.json")
    up_range10_file = os.path.join(report_path, f"{basic_name}_up_range10_{report_date}.json")
...
def get_weekly_reports(date, report_date, base_path='./stock_data/company_data'):
    ...
    get_weekly_report(func, "weekly_report", sh_dict, 1, args, base_path, report_date, individual_file=True)
    get_weekly_report(func, "weekly_report", sz_dict, 0, args, base_path, report_date, individual_file=True)
...
def check_daily_up_interface(date, base_path='./stock_data/company_data', creating_new_dict=True):
    ...
    if creating_new_dict:
        ...
        # 读取生成的新股文件
        sh_dict_path = os.path.join(base_path, "sh_a_stocks_excluding_new.json")
        sz_dict_path = os.path.join(base_path, "sz_a_stocks_excluding_new.json")
        sh_dict = load_json(sh_dict_path)
        sz_dict = load_json(sz_dict_path)
        exclude_limit_up(sh_dict, sz_dict, date)
        sh_dict = load_json(os.path.join(base_path, "sh_a_stocks_excluding_new_and_limit_up.json"))
        sz_dict = load_json(os.path.join(base_path, "sz_a_stocks_excluding_new_and_limit_up.json"))
    else:
        sh_dict = load_json(os.path.join(base_path, "sh_a_stocks_excluding_new_and_limit_up.json"))
        sz_dict = load_json(os.path.join(base_path, "sz_a_stocks_excluding_new_and_limit_up.json"))
        if len(sh_dict) == 0 or len(sz_dict) == 0:  # 上面if分支的代码串
            exclude_new_stock_interface(date)
            sh_dict_path = os.path.join(base_path, "sh_a_stocks_excluding_new.json")
            sz_dict_path = os.path.join(base_path, "sz_a_stocks_excluding_new.json")
            sh_dict = load_json(sh_dict_path)
            sz_dict = load_json(sz_dict_path)
...
def get_limit_up_dict(date, base_path='./stock_data/company_data', excluding=True, relative_path='weekly_report'):
    limit_up_df = pd.DataFrame(columns=['代码', '名称'])
    report_date = date
    ...
    include_prefix = ["600", "601", "603", "605", "000", "001", "002", "003", "300", "301", "688"]
    if excluding:
        include_prefix = ["600", "601", "603", "605", "000", "001", "002", "003"]

    for index, row in stock_zt_pool_em_df.iterrows():
        stock_code = row['代码']
        stock_name = row['名称']

        if any(stock_code.startswith(prefix) for prefix in include_prefix):
            limit_up_df = pd.concat([limit_up_df, pd.DataFrame([{"代码": stock_code, "名称": stock_name}])],
                                    ignore_index=True)
    # print(os.path.join(base_path, relative_path, f"limit_up_dict_{report_date}"))
    save_to_json_v2(limit_up_df, os.path.join(base_path, relative_path, f"limit_up_dict_{report_date}.json"))

def get_limit_up_dict_v2(date, base_path='./stock_data/company_data', excluding=True, relative_path='weekly_report'):
    """
    将沪深的涨停股分离，存在两个文件中。
    ...
    sh_file_path = os.path.join(base_path, relative_path, f"limit_up_dict_sh_{report_date}.json")
    sz_file_path = os.path.join(base_path, relative_path, f"limit_up_dict_sz_{report_date}.json")
...
def exclude_limit_up(sh_dict, sz_dict, date, base_path='./stock_data/company_data', relative_path='weekly_report', exclude_new=True):
        """
        ...
        if exclude_new:
            save_to_json_v2(filtered_sh_df, os.path.join(base_path, "sh_a_stocks_excluding_new_and_limit_up.json"))
            save_to_json_v2(filtered_sz_df, os.path.join(base_path, "sz_a_stocks_excluding_new_and_limit_up.json"))
        else:
            save_to_json_v2(filtered_sh_df, os.path.join(base_path, "sh_a_stocks_excluding_limit_up.json"))
            save_to_json_v2(filtered_sz_df, os.path.join(base_path, "sz_a_stocks_excluding_limit_up.json"))

def exclude_new_stock(sh_dict, sz_dict, base_path='./stock_data/company_data'):
    """
    ...
    save_to_json_v2(filtered_sh_df, os.path.join(base_path, "sh_a_stocks_excluding_new.json"))
    save_to_json_v2(filtered_sz_df, os.path.join(base_path, "sz_a_stocks_excluding_new.json"))
...
def exclude_new_stock_interface(date, base_path='./stock_data/company_data'):
    sh_dict, sz_dict, h_dict = create_dict(base_path, False)
...
if debug:
    ...
    base_path = "../data/stock_data/company_data"
