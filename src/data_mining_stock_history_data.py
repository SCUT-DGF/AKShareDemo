...
def get_company_basic_profile(basic_name, stock_dict, base_path, processed_stocks, flag, begin_date, end_date, stock_sh_a_spot_em_df, stock_sz_a_spot_em_df, interrupt_file):
    """
    获取每日报表
    ...
    for i, stock in enumerate(stock_dict):
        ...
        try:
            ...
            # 新增“涨跌”列
            stock_hist_df['涨跌'] = stock_hist_df.apply(lambda row: 1 if row['收盘'] > row['开盘'] else 0, axis=1)
